import {state,EVENT_DATE,ADMIN_KEY,roleById,peopleFor,scoringStations,scoreKey,scoreFor,totalFor,rankedParticipants} from "./state.js?v=10.12.8";
import {esc,cleanPhone,toast,downloadCSV} from "./utils.js?v=10.12.8";
import {db,collection,addDoc,deleteDoc,doc,setDoc,serverTimestamp} from "./firebase.js?v=10.12.8";
...
const assignedHelper=()=>{
  ...
  if(helperId){
    const byId=state.helpers.find(h=>h.id===helperId);
    if(byId)return byId;
  }
  const phone=cleanPhone(localStorage.getItem("rvn_helper_phone")||"");
  return phone?state.helpers.find(h=>cleanPhone(h.phone)===phone)||null:null;
};

function normalizeRoute(value){
  const route=String(value||"").trim().toLowerCase().replace(/ß/g,"ss");
  if(route==="7 km"||route==="7km"||route.includes("kleine"))return "7 km";
  if(route==="17 km"||route==="17km"||route.includes("grosse")||route.includes("große"))return "17 km";
  return "";
}
function routeGpxPath(value){
  const route=normalizeRoute(value);
  if(route==="7 km")return "strecken/strecken_kleine_runde.gpx";
  if(route==="17 km")return "strecken/strecken_grosse_runde.gpx";
  return "";
}
...
<section class="grid main-sections">
  ...
  <article class="card secure-card helper-card"><div class="icon">🔒</div><h3>Helferbereich</h3><p>Geschützter Zugang zu Einsatzplan, Stationen und Helferinformationen.</p><a href="#helfer" class="btn helper-open-btn" data-open-helper="true" role="button">Helferbereich öffnen</a></article>
...
<section class="location-paddock-row">
  ...
  <article class="panel ceremony-card">
    <div class="head"><div><h2>🏆 Abendessen &amp; Siegerehrung</h2><p class="sub">Gemeinsamer Abschluss des Veranstaltungstages</p></div></div>
    <p>🍽️ Das gemeinsame Abendessen beginnt <strong>voraussichtlich um 18:30 Uhr.</strong></p>
    <p>Im Anschluss findet die <strong>Siegerehrung</strong> statt.</p>
    <p>🎁 Die Ehrenpreise werden über einen <strong>Ehrenpreistisch</strong> vergeben. Die Auswahl erfolgt <strong>von hinten nach vorne entsprechend der Rangliste</strong> – der letzte Platz wählt zuerst, der Sieger zuletzt.</p>
    <p>📢 Der genaue Beginn der Siegerehrung wird rechtzeitig bekannt gegeben.</p>
  </article>
...
function helperPage(){
  ...
  return `${helperAccessPanel()}<section class="panel">
    <div class="head"><div><h2>👥 Stations- und Helferübersicht</h2><p class="sub">Alle Stationen, Aufgaben und eingetragenen Helfer auf einen Blick.</p></div></div>
    <div class="cards">
      ${state.roles.map(r=>{
        const count=peopleFor(r.id).length;
        const free=Math.max(0,Number(r.max||0)-count);
        return `<div class="info"><h3>${r.icon} ${esc(r.name)}</h3><p>${esc(r.description||"Beschreibung folgt.")}</p>${r.location?`<p><strong>📍 Standort:</strong> ${esc(r.location)}</p>`:""}${r.dutyTime?`<p><strong>🕒 Einsatzzeit:</strong> ${esc(r.dutyTime)}</p>`:""}${r.contact?`<p><strong>👤 Ansprechpartner:</strong> ${esc(r.contact)}</p>`:""}<p><strong>${count} / ${r.max}</strong> eingetragen · ${free===0?"voll":free+" frei"}</p>${peopleFor(r.id).length?`<p><strong>Helfer:</strong><br>${peopleFor(r.id).map(h=>esc(h.name)).join("<br>")}</p>`:`<p class="sub">Noch keine Helfer eingetragen.</p>`}<button class="btn ${free===0?"light":"alt"}" ${free===0?"disabled":""} onclick="selectHelperRole('${r.id}')">${free===0?"Voll":"Diesen Bereich wählen"}</button></div>`;
      }).join("")}
    </div>
  </section>
  <section class="panel"><h2>📝 Helferanmeldung</h2><div class="notice">Die Telefonnummer wird später zugleich für den Zugang zu deiner Station, der Meldestelle oder dem Springerbereich verwendet.</div><form id="helperSignupForm" class="form"><label>Name<input id="helperName" required placeholder="Vor- und Nachname"></label><label>Telefonnummer<input id="helperPhone" required placeholder="Telefonnummer"></label><label>Bereich<select id="helperRole">${state.roles.map(r=>`<option value="${r.id}">${r.icon} ${esc(r.name)}</option>`).join("")}</select></label><label>Zeitraum<select id="helperTime"><option>ganztags</option><option>vormittags</option><option>nachmittags</option><option>abends</option><option>nach Absprache</option></select></label><label class="full">Bemerkung<textarea id="helperNote" placeholder="z. B. mit Auto, erst ab 12 Uhr"></textarea></label><button class="btn full" type="submit">Verbindlich eintragen</button></form></section>
...
function participantPage(){
  ...
  const route=normalizeRoute(p.routeType||p.route);
  ...
  const routeUrl=p.routeGpxUrl||routeGpxPath(route);
  ...
  <section class="panel"><h2>🏅 Meine Gesamtpunktzahl</h2><div class="notice">Aktuelle Gesamtpunktzahl: <strong>${totalFor(p.id)} Punkte</strong>.<br>Einzelpunkte pro Station und bereits gelöste Aufgaben werden hier nicht angezeigt.</div></section>`;
}

function isRouteReleased(p){return Boolean(p.gpxReleased||p.routeReleased);}

function statusOptions(current){
  const status={"gemeldet":"Gemeldet","gestartet":"Gestartet","im Ziel":"Im Ziel"}[current]||current||"Gemeldet";
  return ["Gemeldet","Startvorbereitung","Startbereit","Gestartet","Im Ziel"].map(v=>`<option value="${v}" ${status===v?"selected":""}>${v}</option>`).join("");
}
function routeOptions(current){const route=normalizeRoute(current);return `<option value="">Bitte wählen</option><option value="7 km" ${route==="7 km"?"selected":""}>7 km – Kleine Runde</option><option value="17 km" ${route==="17 km"?"selected":""}>17 km – Große Runde</option>`}
...
function stationPage(){
  ...
  if(!id||!id.startsWith("station-"))return `<section class="panel"><h2>🐴 Eigene Station</h2><div class="notice">Bitte zuerst im Helferbereich mit deiner hinterlegten Telefonnummer anmelden.</div></section>`;
...
function resultsPage(){
  if(state.isAdmin){
    ...
    return `<section class="panel"><div class="head"><div><h2>🏆 Ergebnisübersicht</h2><p class="sub">Vollständige Ansicht für den Admin.</p></div></div>${rank.length?rank.map((p,i)=>`<div class="entry"><div><strong>${i+1}. ${esc(p.name)}</strong><br><small>Team: ${esc(p.horse||"-")}</small></div><strong>${totalFor(p.id)} Punkte</strong></div>`).join(""):`<div class="notice">Noch keine Ergebnisse vorhanden.</div>`}</section>`;
  }
  const p=currentParticipant();
  if(!p)return `<section class="panel"><h2>🏆 Meine Gesamtpunktzahl</h2><div class="notice">Bitte öffne zuerst den Teilnehmerbereich und melde dich mit Startnummer und Teamname an. Eine öffentliche Rangliste wird nicht angezeigt.</div><button class="btn" onclick="go('teilnehmer')">Zum Teilnehmerbereich</button></section>`;
  return `<section class="panel"><h2>🏆 Meine Gesamtpunktzahl</h2><div class="notice"><strong>${esc(p.name)}</strong><br>Gesamt: <strong>${totalFor(p.id)} Punkte</strong><br><br>Einzelpunkte pro Station, gelöste Aufgaben und Ergebnisse anderer Teilnehmer werden nicht angezeigt.</div></section>`;
}
...
function helperPhoneLogin(e){
  ...
  const matches=state.helpers.filter(h=>cleanPhone(h.phone)===cleaned);
  if(!cleaned)return toast("Bitte die hinterlegte Telefonnummer eingeben.");
  ...
  localStorage.setItem("rvn_station_access",role);
  toast("Willkommen "+(helper.name||"im Helferbereich")+"!");
  window.go(role==="meldestelle"?"meldestelle":role==="springer-kfz"?"springer":"station");
}
window.closeHelperGate=()=>{sessionStorage.removeItem("rvn_helper_gate");localStorage.removeItem("rvn_helper_id");localStorage.removeItem("rvn_helper_phone");localStorage.removeItem("rvn_station_access");state.page="helfer";window.renderApp()};
...
async function accessLogin(e){e.preventDefault();const password=document.getElementById("accessPassword").value,role=document.getElementById("accessRole").value,phone=document.getElementById("accessPhone").value;if(password!=="Helfer")return toast("Falsches Helferpasswort.");const helper=state.helpers.find(h=>h.role===role&&cleanPhone(h.phone)===cleanPhone(phone));if(!helper)return toast("Keine passende Helferanmeldung gefunden.");localStorage.setItem("rvn_helper_id",helper.id);localStorage.setItem("rvn_helper_phone",phone);localStorage.setItem("rvn_station_access",role);sessionStorage.setItem("rvn_helper_gate","yes");toast("Zugang freigeschaltet.");window.go(role==="meldestelle"?"meldestelle":role==="springer-kfz"?"springer":"station")}
...
async function addParticipant(e){
  e.preventDefault();
  const route=normalizeRoute(document.getElementById("pRoute").value);
  await addDoc(collection(db,"participants"),{
    startNumber:document.getElementById("pStart").value.trim(),
    name:document.getElementById("pName").value.trim(),
    horse:document.getElementById("pHorse").value.trim(),
    startTime:document.getElementById("pTime").value,
    paddock:document.getElementById("pPaddock").value.trim(),
    route,routeType:route,status:"Gemeldet",
    routeReleased:false,gpxReleased:false,qrReleased:false,
    createdAt:serverTimestamp()
  });
  e.target.reset();
...
async function sendAlert(e){
  ...
  if((priority==="help"||priority==="emergency")&&!message)return toast("Bei Unterstützung oder Notfall ist eine Beschreibung erforderlich.");
  await addDoc(collection(db,"alerts"),{
    stationId:document.getElementById("alertStation").value,
    priority,
    text:message||(priority==="ok"?"Alles in Ordnung":""),
    status:"offen",
    createdAt:serverTimestamp()
  });
  e.target.reset();
...
window.saveParticipantMeta=async id=>{
  if(!canManageParticipants())return toast("Keine Berechtigung.");
  const route=normalizeRoute(document.getElementById("route-"+id).value);
  await setDoc(doc(db,"participants",id),{
    startNumber:document.getElementById("start-"+id).value,
    name:document.getElementById("name-"+id).value,
    horse:document.getElementById("horse-"+id).value,
    startTime:document.getElementById("time-"+id).value,
    paddock:document.getElementById("paddock-"+id).value,
    route,routeType:route,
    status:document.getElementById("status-"+id).value,
    routeMapUrl:document.getElementById("map-"+id).value,
    updatedAt:serverTimestamp()
  },{merge:true});
  toast("Gespeichert.");
}
window.confirmStartReady=async id=>{
  if(!canManageParticipants())return toast("Keine Berechtigung.");
  const participant=state.participants.find(p=>p.id===id);
  const route=normalizeRoute(document.getElementById("route-"+id)?.value||participant?.routeType||participant?.route);
  if(!route)return toast("Bitte zuerst 7 km oder 17 km auswählen.");
  const routeGpxUrl=routeGpxPath(route);
  try{
    const response=await fetch(routeGpxUrl,{method:"GET",cache:"no-store"});
    if(!response.ok)throw new Error(`GPX nicht erreichbar (${response.status})`);
  }catch(error){
    console.error("GPX-Prüfung fehlgeschlagen",error);
    return toast(`Die GPX-Datei für ${route} ist nicht erreichbar. Freigabe wurde nicht gespeichert.`);
  }
  await setDoc(doc(db,"participants",id),{
    route,routeType:route,routeGpxUrl,
    routeReleased:true,gpxReleased:true,qrReleased:true,
    status:"Startbereit",releasedAt:serverTimestamp(),routeReleasedAt:serverTimestamp(),
    updatedAt:serverTimestamp()
  },{merge:true});
  toast(`Startbereitschaft bestätigt – ${route} wurde freigegeben.`);
}
...
window.helperLogout=()=>{localStorage.removeItem("rvn_helper_id");localStorage.removeItem("rvn_helper_phone");localStorage.removeItem("rvn_station_access");sessionStorage.removeItem("rvn_helper_gate");state.page="helfer";toast("Du wurdest aus dem Helferbereich abgemeldet.");window.renderApp()}
...
window.openStationNavigation=id=>{
  const station=roleById(id);
  if(!station||!station.location)return toast("Für diese Station ist noch kein Standort hinterlegt.");
  const location=String(station.location).trim();
  const url=/^https?:\/\//i.test(location)?location:`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(location)}`;
  window.open(url,"_blank","noopener");
};
window.filterStationParticipants=q=>{const value=String(q||"").trim().toLowerCase();document.querySelectorAll(".station-participant").forEach(el=>{el.style.display=!value||String(el.dataset.search||"").includes(value)?"":"none";});};
