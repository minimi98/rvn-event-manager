    ...
    ${state.roles.filter(r=>r.id.startsWith("station-")).map(r=>`
      <div class="entry">
        <div style="width:100%">
          ...
          <button class="btn light" onclick="saveStationInfo('${r.id}')">Station speichern</button>
        </div>
      </div>`).join("")}
  </section>`;
}
