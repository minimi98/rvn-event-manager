...
const routeUrlFor=p=>p.routeGpxUrl||(p.routeType==="large"?state.settings.largeRouteGpxUrl:state.settings.smallRouteGpxUrl)||"";
const routeLengthFor=p=>p.route||(p.routeType==="large"?state.settings.largeRouteLength:state.settings.smallRouteLength)||state.settings.routeLength||"";
const qrUrl=url=>`https://api.qrserver.com/v1/create-qr-code/?size=220x220&margin=10&data=${encodeURIComponent(url)}`;

export function shell(content){
return `<div class="shell"><header class="topbar"><div class="brand"><img class="logo" src="assets/logo.jpg" alt="RVN Logo"><div><h1>RVN Event Manager</h1><p>Beach Please – wir reiten! · O-Ritt 2026</p></div></div><button class="menu" onclick="go('home')" aria-label="Startseite">⌂</button></header><main class="main">${content}</main><nav class="bottom">${nav("home","🏠","Home")}${nav("oritt","🐴","O-Ritt")}${nav("helfer","🙋","Helfer")}${nav("zugang","🔑","Zugang")}${nav("admin","👑","Admin")}</nav></div>`}
...
function homePage(){return `<section class="hero beach-hero"><div class="hero-copy"><div class="kicker">Reit- und Fahrverein Neuendettelsau e.V.</div><h2>Beach Please<br><span>wir reiten!</span></h2><p>Orientierungsritt am 25. Juli 2026</p><div class="chip">Sommer · Sonne · Sattel</div></div><div class="beach-sign"><strong>RVN</strong><span>O-Ritt 2026</span></div></section><section class="grid two"><article class="card beach-card"><div class="icon">🐴</div><h3>O-Ritt 2026</h3><p>Alle Informationen zur Veranstaltung, Teilnehmerzugang und Ergebnisse.</p><button class="arrow" onclick="go('oritt')">›</button></article><article class="card beach-card"><div class="icon">🔐</div><h3>Zugänge</h3><p>Geschützte Zugänge für Helfer, Stationen, Meldestelle und Springer.</p><button class="arrow" onclick="go('zugang')">›</button></article></section>`}
...
function helperPage(){
  if(!helperGateOpen()) return `<section class="panel helper-gate"><h2>🔒 Helferbereich</h2><p class="sub">Bitte zuerst das Helfer-Passwort eingeben.</p><form id="helperGateForm" class="form"><label>Passwort<input id="helperGatePassword" type="password" autocomplete="current-password"></label><button class="btn full">Helferbereich öffnen</button></form></section>`;
...
function participantPage(){
  ...
  <section class="panel"><h2>🧭 Meine Strecke</h2>${release?(()=>{const url=routeUrlFor(p);return url?`<div class="route-box"><div><h3>${p.routeType==="large"?"Große Runde":"Kleine Runde"}</h3><p><strong>Streckenlänge:</strong> ${esc(routeLengthFor(p))}</p><p><a class="btn alt" href="${esc(url)}" target="_blank" rel="noopener">GPX herunterladen / in OsmAnd öffnen</a></p>${p.routeMapUrl?`<p><a class="btn light" href="${esc(p.routeMapUrl)}" target="_blank" rel="noopener">Kartenausschnitt öffnen</a></p>`:""}</div><img class="route-qr" src="${qrUrl(url)}" alt="QR-Code zur GPX-Strecke"></div>`:`<div class="notice">Noch kein Streckenlink hinterlegt.</div>`})():`<div class="notice">Die Reitkarte wird 30 Minuten vor deiner Startzeit freigeschaltet.</div>`}</section>
...
function adminExtras(){
  return `<section class="panel">
    ...
    <form id="eventInfoForm" class="form">
      <label>Allgemeine Streckenlänge<input id="routeLength" value="${esc(state.settings.routeLength||"17 km")}"></label><label>Kleine Runde – Länge<input id="smallRouteLength" value="${esc(state.settings.smallRouteLength||"")}"></label><label class="full">Kleine Runde – GPX-Link<input id="smallRouteGpxUrl" value="${esc(state.settings.smallRouteGpxUrl||"")}"></label><label>Große Runde – Länge<input id="largeRouteLength" value="${esc(state.settings.largeRouteLength||"")}"></label><label class="full">Große Runde – GPX-Link<input id="largeRouteGpxUrl" value="${esc(state.settings.largeRouteGpxUrl||"")}"></label>
...
async function saveEventInfo(e){e.preventDefault();if(!state.isAdmin)return toast("Keine Berechtigung.");await setDoc(doc(db,"settings","main"),{routeLength:document.getElementById("routeLength").value,smallRouteLength:document.getElementById("smallRouteLength").value,smallRouteGpxUrl:document.getElementById("smallRouteGpxUrl").value,largeRouteLength:document.getElementById("largeRouteLength").value,largeRouteGpxUrl:document.getElementById("largeRouteGpxUrl").value,participantGeneralInfo:document.getElementById("participantGeneralInfo").value},{merge:true});toast("Veranstaltungsinformationen gespeichert.");}
...
function helperGateLogin(e){e.preventDefault();if(document.getElementById("helperGatePassword").value==="Helfer"){sessionStorage.setItem("rvn_helper_gate","yes");window.renderApp()}else toast("Falsches Helfer-Passwort.")}
