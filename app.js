...
const defaultRoles = [
  ...Array.from({ length: 9 }, (_, i) => ({ id: `station-${i + 1}`, name: `Station ${i + 1}`, icon: "🐴", max: 5, description: "" })),
  { id: "haengerplatz", name: "Hängerplatz", icon: "🚗", max: 4, description: "Einweisung und Ordnung am Hängerparkplatz." },
  { id: "grillen", name: "Grillteam", icon: "🔥", max: 6, description: "Unterstützung beim Grillen und Abspülen am Abend." },
  { id: "meldestelle", name: "Meldestelle", icon: "📋", max: 4, description: "Anmeldung, Startunterlagen und Rückfragen." },
  { id: "aufbau", name: "Aufbauteam", icon: "🔧", max: 6, description: "Aufbau vor der Veranstaltung." },
  { id: "abbau", name: "Abbauteam", icon: "🔨", max: 6, description: "Abbau nach der Veranstaltung." },
  { id: "springer-kfz", name: "Springer mit KFZ", icon: "🚙", max: 4, description: "Kontrolle der Stationen mit Fahrzeug." }
];
...
async function addRole(e) {
  ...
  await setDoc(doc(db, "roles", slug(name)), { name, icon, max, description, createdAt: serverTimestamp() });
  e.target.reset();
...
function init() {
  try {
    ...
    onSnapshot(query(collection(db, "roles"), orderBy("createdAt", "asc")), snap => { const custom = snap.docs.map(d => ({ id: d.id, ...d.data() })); roles = custom.length ? custom : defaultRoles; render(); });
