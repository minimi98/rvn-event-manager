...
let currentEventName = "O-Ritt 2026";
let eventArchive = [
  { id: "oritt-2026", name: "O-Ritt 2026", type: "O-Ritt", active: true },
  { id: "turnier-2026", name: "Turnier 2026", type: "Turnier", active: false }
];
let helpers = [];
let news = [];
...
function shell(content) {
  return `<div class="shell">
    <header class="topbar">
      <div class="brand"><button class="menu" onclick="go('home')">☰</button><div><h1>RVN Event Manager</h1><p>${esc(currentEventName)} · O-Ritt & Turnier</p></div></div>
...
function meldestellePage() {
  const sorted = [...participants].sort((a, b) => Number(a.startNumber || 9999) - Number(b.startNumber || 9999));
  return `<section class="panel">
    <div class="head">
      <div>
        <h2>📋 Meldestelle</h2>
        <p class="sub">Teilnehmer, Startnummern, Pferde und Startstatus verwalten.</p>
      </div>
      ${isAdmin ? `<button class="btn alt" onclick="exportParticipantsCSV()">Teilnehmer CSV</button>` : ""}
    </div>
    ${isAdmin ? participantForm() : `<div class="notice">Die Teilnehmerverwaltung ist nur im Adminbereich sichtbar.</div>`}
    <h3>Teilnehmerliste</h3>
    ${sorted.length ? sorted.map(p => `<div class="entry">
      <div>
        <strong>${esc(p.startNumber || "-")} · ${esc(p.name)}</strong><br>
        <small>Pferd: ${esc(p.horse || "-")} · Strecke: ${esc(p.route || "-")} · Status: ${statusLabel(p.status)}</small>
      </div>
      ${isAdmin ? `<div style="display:flex;gap:8px;flex-wrap:wrap">
        <button class="btn light" onclick="setParticipantStatus('${p.id}','registered')">Gemeldet</button>
        <button class="btn light" onclick="setParticipantStatus('${p.id}','started')">Gestartet</button>
        <button class="btn light" onclick="setParticipantStatus('${p.id}','finished')">Im Ziel</button>
        <button class="btn danger" onclick="deleteParticipant('${p.id}')">Löschen</button>
      </div>` : ""}
    </div>`).join("") : `<p class="sub">Noch keine Teilnehmer eingetragen.</p>`}
  </section>`;
}
  ...
  return `<section class="panel">
    <div class="head">
      <div>
        ...
        <p class="sub">Vorbereitet für O-Ritte und Turniere je Jahr. Die aktuelle Version arbeitet noch mit der aktiven Veranstaltung: <span class="accent">${esc(currentEventName)}</span>.</p>
      </div>
    </div>
    <div class="notice">
      Dieser Bereich ist absichtlich schlank gehalten und ersetzt nicht die offizielle Homepage. Er dient nur zur Organisation von O-Ritt und Turnier.
    </div>
    <div class="cards">
      ${eventArchive.map(ev => `<div class="info"><h3>${ev.type === "O-Ritt" ? "🐴" : "🏇"} ${esc(ev.name)}</h3><p>${ev.active ? "Aktive Veranstaltung" : "Vorbereitet / Archiv"}</p></div>`).join("")}
    </div>
  </section>`;
}
...
function render() {
  ...
  else if (page === "meldestelle") content = meldestellePage();
  else if (page === "ergebnisse") content = resultsPage();
  else if (page === "archiv") content = eventArchivePage();
...
async function addParticipant(e) {
  e.preventDefault();
  const payload = { startNumber: document.getElementById("participantStart").value.trim(), name: document.getElementById("participantName").value.trim(), horse: document.getElementById("participantHorse").value.trim(), route: document.getElementById("participantRoute").value, status: "registered", createdAt: serverTimestamp() };
...
window.setParticipantStatus = async (id, status) => {
  if (!isAdmin) return toast("Bitte Admin einloggen.");
  await setDoc(doc(db, "participants", id), { status, updatedAt: serverTimestamp() }, { merge: true });
  toast("Teilnehmerstatus gespeichert.");
};

window.exportParticipantsCSV = () => {
  if (!isAdmin) return toast("Bitte Admin einloggen.");
  const rows = [["Startnummer", "Teilnehmer", "Pferd", "Strecke", "Status"]];
  [...participants]
    .sort((a, b) => Number(a.startNumber || 9999) - Number(b.startNumber || 9999))
    .forEach(p => rows.push([p.startNumber || "", p.name || "", p.horse || "", p.route || "", statusLabel(p.status)]));
